...
In the second constructor a [`GFit.λFunct`](@ref) object is accepted, as generated by the [`@λ`](@ref) macro).  The anonymous function is typically a mathematical expression combining any number of parameters and/or other component evaluations within the same model.  The expression should be given in the form:
```
