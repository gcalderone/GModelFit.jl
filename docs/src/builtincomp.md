...
The `FComp` component is a wrapper to use a standard Julia function as actual implementation for a component evaluation.  It comes in two flavours: `FComp` (discussed here) based on one or more named scalar parameters, and `FCompv` (see next section) based on a vector of unnamed parameters.  The `FComp` constructors are defined as follows:

```julia
FComp(funct::Function, deps=Symbol[]; par1=guess1, par2=guess2, ...)
FComp(funct::FunctDesc)
```
In the first constructor, where `funct` is the Julia function, `deps` is a vector of dependencies (either the domain dimensions or other components) and `par1`, `par2` etc. are the named parameters with their corresponding initial guess values.
...
In the second constructor a [`GFit.FunctDesc`](@ref) object is accepted, as generated by the [`@λ`](@ref) macro).  The anonymous function is typically a mathematical expression combining any number of parameters and/or other component evaluations within the same model.  The expression should be given in the form:
```
